...
- Add the SQLAlchemy model under `backend/app/db/models/reconciliation.py`.
- Add the repository under `backend/app/repositories/reconciliation.py`.
- Initialize Alembic with `uv run alembic init migrations` when the project has
  no Alembic scaffold yet.
- Keep Alembic scaffold files under `backend/migrations/`; manually edit
  `migrations/env.py` only to load app settings, point `target_metadata` at the
  SQLAlchemy model metadata, and support injected test connections.
- Generate the first migration from the SQLAlchemy model with
  `uv run alembic revision --autogenerate -m "create reconciliation cases"`,
  then review the generated file before applying it.
- Add minimal typed persistence contracts under
  `backend/app/domain/reconciliation/contracts.py`.
...
class BaseReconciliationCaseRepository:
    ...
    def list(
        ...
        Why: M1.5 list endpoints need a repository query that remains local
    ...
    def get(self, case_id: UUID) -> BaseReconciliationCase | None:
        ...
        Why: M1.5 detail endpoints need a not-found-safe repository lookup.
...
Backend local settings live in `backend/.env`. Start from the example file:
...
```bash
# Run only once when the backend has no Alembic scaffold yet.
uv run alembic init migrations

# After SQLAlchemy models exist, generate the migration from model metadata.
uv run alembic revision --autogenerate -m "create reconciliation cases"
...
- [../../../backend/.env.example](../../../backend/.env.example) (new): Provides the local backend PostgreSQL URL for L1.
...
- `backend/migrations/script.py.mako` (new): Command-generated Alembic revision template for L2.
- `backend/migrations/env.py` (new): Wires migrations to app metadata for L2.
- `backend/migrations/versions/*_create_reconciliation_cases.py` (new): Autogenerated Alembic revision, reviewed for the Base API case table for L3.
- `backend/app/db/session.py` (new): Adds engine/session helpers for L4.
- `backend/app/domain/reconciliation/contracts.py` (new): Adds typed persistence DTOs and status values for L5.
- `backend/app/db/models/reconciliation.py` (new): Adds the SQLAlchemy model for L5.
- `backend/app/repositories/reconciliation.py` (new): Adds create/list/get storage behavior for L6.
